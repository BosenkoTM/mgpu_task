...
*   $\pi(a_t | s_t)$ — **политика оркестрации (Policy)**: алгоритм, принимающий решение на каждом шаге. Переменные $N$ (итоговое число агентов) и $I$ (число циклов) теперь являются не входными параметрами, а *производными характеристиками конкретной траектории $\tau$*.

**2. Функция стоимости (Cost Vector):**
Стоимость больше не является скалярной. Это вектор затрат:
$$ C(\tau) = \big(C_{\text{tok}}(\tau), C_{\text{\$}}(\tau), L(\tau), C_{\text{tool}}(\tau)\big) $$
...
Найти такую политику $\pi^*$, которая максимизирует ожидаемое качество $\mathbb{E}[Q(\tau)]$, соблюдая жесткие бюджетные ограничения и контролируя хвостовые риски (SLA):
$$ \max_{\pi} \mathbb{E}_{\tau \sim \pi} [Q(\tau)] $$
**При условиях:**
1. $\mathbb{E}[C_{\text{\$}}(\tau)] \le B_{\text{max}}$ (лимит бюджета).
2. $\mathbb{E}[L(\tau)] \le L_{\text{max}}$ (лимит времени).
3. $\Pr\big(Q(\tau) < Q_{\min} \mid z(x)\big) \le \alpha_{\text{risk}}$ (ограничение на долю критических провалов в зависимости от класса сложности задачи $z(x)$).
...
$$ \frac{\widehat{\Delta Q}_{t+1}}{\widehat{\Delta C}_{t+1}} < \lambda \quad \text{или} \quad \Pr(\Delta Q_{t+1} > \varepsilon \mid s_t) < \delta $$

---

### ЧАСТЬ 2. Необходимые данные и архитектура решения

Для решения этой задачи классический Grid Search не подойдет. Потребуется трехуровневая архитектура оптимизации и специфический сбор данных.

**Требуемые данные (Trace-centric Schema):**
