...
**Результат.** График показывает область компромисса между качеством и токенными затратами для статических конфигураций. Выбранная точка `N = 3`, `I = 1` обеспечивает требуемый уровень качества при минимальной средней стоимости среди допустимых вариантов.
...
**Результат.** Распределения качества у двух моделей сопоставимы: динамическая политика незначительно уступает по среднему значению, но имеет более высокий 10-й процентиль, то есть устойчивее работает в нижней части распределения.
...
**Результат.** Динамическая модель заметно снижает типичные токенные затраты, поскольку завершает простые запросы раньше. При этом отдельные сложные запросы могут получать больший бюджет, что объясняет близкие значения верхнего хвоста распределения.
...
**Результат.** Число агентов в динамической модели не фиксировано: для большинства запросов используется меньше трех агентов, а дополнительные вызовы выполняются только тогда, когда ожидаемый прирост качества оправдывает затраты.
