...
Показатель $`G`$ влияет на итоговый $`Q_w`$, но в текущем коде не имеет отдельного жесткого порога. Для нормативных запросов в рабочей системе рекомендуется добавить условие $`G\ge g_{\min}`$ и проверку актуальности реквизитов источника.
