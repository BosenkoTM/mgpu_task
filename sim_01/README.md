...
flowchart TD
    ...
    U --> D{Качество достигнуто или предельная полезность мала?}
    D -- Нет --> A
    D -- Да --> S
...
```math
a_t \in \{\operatorname{RAG},\operatorname{CALL}(j),\operatorname{SYNTHESIZE},\operatorname{STOP}\},
...
где $`\mathcal{C}(\tau)`$ — множество облачных API-вызовов координатора, субагентов и финального синтезатора. Если требуется оптимизировать денежную стоимость, используется
...
где $`p^{\mathrm{in}}_t`$ и $`p^{\mathrm{out}}_t`$ — тарифы за миллион входных и выходных токенов. Такое раздельное суммирование соответствует правилам учета API-токенов, приведенным в документации DeepSeek [1].
...
```math
\mathbf{q}(y,x)=
\left(C_{\mathrm{req}},F,G,A_{\mathrm{ped}},O_{\mathrm{fit}}\right),
```

где $`C_{\mathrm{req}}`$ — покрытие требований преподавателя, $`F`$ — фактическая корректность, $`G`$ — подтвержденность источниками, $`A_{\mathrm{ped}}`$ — педагогическая применимость, а $`O_{\mathrm{fit}}`$ — соответствие требуемому объему и формату ответа с учетом контекстного окна. Компоненты фактической корректности, релевантности и подтвержденности контекстом согласуются с многомерной оценкой RAG-систем в RAGAS и ARES [6, 7]; покрытие требований и педагогическая применимость добавлены как предметно-ориентированные критерии вузовского сценария.

В основной постановке ответ считается точным не по среднему баллу, а при выполнении набора обязательных ограничений:

```math
\operatorname{Exact}(y,x)=1
\iff
\begin{cases}
C_{\mathrm{req}}\ge c_{\min},\\
F\ge f_{\min},\\
G\ge g_{\min},\\
A_{\mathrm{ped}}\ge a_{\min},\\
O_{\mathrm{fit}}\ge o_{\min}.
\end{cases}
```
...
```math
\Pr_{\tau\sim\pi}
\left(\operatorname{Exact}(y_\tau,x)=1\mid z(x)=c\right)
\ge 1-\alpha_c,
...
```math
Q_{w}=w_C C_{\mathrm{req}}+w_F\,F+w_G\,G+w_A\,A_{\mathrm{ped}},
\qquad
w_C+w_F+w_G+w_A=1,
\qquad w_j\ge0.
```
...
которая задает следующий приоритет: сначала покрытие требований преподавателя, затем фактическая корректность, подтвержденность источниками и педагогическая применимость. Эти веса нужны только для воспроизводимого частного расчета и сравнения стратегий; они не доказывают, что такое соотношение оптимально для реального вуза.
...
Показатели $`G`$ и $`A_{\mathrm{ped}}`$ влияют на итоговый $`Q_w`$, но в текущем коде не имеют отдельных жестких порогов. Для нормативных запросов в рабочей системе рекомендуется добавить условие $`G\ge g_{\min}`$ и проверку актуальности реквизитов источника.

Для калибровки весов на реальных данных необходимо собрать ответы, оцененные преподавателями по четырем компонентам, и бинарное решение $`r_n\in\{0,1\}`$ «ответ принят / требуется доработка». Затем параметры можно оценить, например, логистической моделью

```math
\Pr(r_n=1\mid\mathbf{q}_n)=
\sigma\!\left(\beta_0+\beta_C C_{\mathrm{req},n}+\beta_F F_n+\beta_G G_n+\beta_A A_{\mathrm{ped},n}+\beta_O O_{\mathrm{fit},n}\right),
```
